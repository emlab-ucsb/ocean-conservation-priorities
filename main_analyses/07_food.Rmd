...
# stocks_info <- read_csv(file.path(emLab_project_dir,
#                                   "data", "02_processed", "food_provision", "MegaData_UncertaintyAnalysis.csv")) %>%
...
```{r}
ranking_a2 <- ranking %>% select(-pick_order)
...
map_priorities(ranking_raster_a2, discrete = T, mpas_raster = MPAs, caption = "Food provision, full effort displacement", 
               ocean_df = ocean_matrix) +
  ggsave(file.path(variant_dir, "priority_map_a2.png"),
         dpi = 300,  height = 4, width = 8)
```
