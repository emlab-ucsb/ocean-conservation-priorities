...
variant_dir <- file.path(emLab_project_dir, "data", "03_output", variant_name, "ray")
...
```{r}
stocks_info %>% 
  mutate(bau = ex_rate*(m*k*(1 - 0)/(ex_rate*0 + m)*(1 - ex_rate*(1 - 0)*m/((ex_rate*0 + m)*r)))) %>% 
  summarize(sum(bau))
```
...
write_rds(ranking,
          file.path(variant_dir, "ranking_results_a1_update_10by10.rds"))
```
...
write_rds(ranking,
        file.path(variant_dir, "ranking_results_a2_update_10by10.rds"))
```
...
```{r}
ranking_a1 <- read_rds(file.path(variant_dir, "ranking_results_a1_update_10by10.rds")) %>% 
  select(-pick_order)
...
```{r}
max(results_per_pixel_a1$delta_catch_a1) -
max(results_per_pixel_a1$delta_catch_a1[results_per_pixel_a1$is_mpa == T])
```
...
```{r}
ranking_a2 <- read_rds(file.path(variant_dir, "ranking_results_a2_update_10by10.rds")) %>% 
  select(-pick_order)
...
max(results_per_pixel_a2$delta_catch_a2)/10^6
max(results_per_pixel_a2$delta_catch_a2) - max(results_per_pixel_a2$delta_catch_a2[results_per_pixel_a2$is_mpa == T])
```
...
write_rds(results_per_pixel, file.path(variant_dir, "results_per_pixel_update.rds"))
```
...
(summary_of_results %>% 
  ...
  write_rds(file.path(variant_dir, "summary_of_food_results_update.rds")))
```

```{r}
read_rds(file.path(variant_dir, "summary_of_food_results.rds"))
```
```{r}
read_rds(file.path(variant_dir, "summary_of_food_results_update.rds"))
```
...
map_priorities(ranking_raster_a1, discrete = T, mpas_raster = MPAs, caption = "Food provision, no effort displacement", 
               ocean_df = ocean_matrix) 
  ggsave(file.path(variant_dir, "priority_map_a1.png"),
         dpi = 300,  height = 4, width = 8)

ranking_raster_a2 <- raster(file.path(variant_dir, "ranking_raster_a2.tif"))

map_priorities(ranking_raster_a2, discrete = T, mpas_raster = MPAs, caption = "Food provision, full effort displacement", 
               ocean_df = ocean_matrix) +
  ggsave(file.path(variant_dir, "priority_map_a2_updated.png"),
         dpi = 300,  height = 4, width = 8)
```
...
ggsave(ranking_diff_map, 
       filename = file.path(emLab_project_dir, "figures", "supplement", "food_priorities_diff_a1_vs_a2.png"), 
       dpi = 300,  height = 9, width = 8)
```


```{r}
cowplot::plot_grid(ranking_diff_map, delta_catch_curve_comparison_plot,
                   ncol = 1, rel_heights = c(2,1), labels = "auto")+
  cowplot::ggsave2(dpi = 300, 
                   filename = file.path(emLab_project_dir, "figures", "supplement", "compare_a1_vs_a2.png"))
```
...
```{r}
delta_catch_per_stock_per_step_a1 <- read_rds(file.path(variant_dir, "delta_catch_per_stock_per_step_a1.rds"))
...
delta_catch_per_stock_per_step_a2 <- read_rds(file.path(variant_dir, "delta_catch_per_stock_per_step_a2.rds"))
...
protection_and_delta_H_per_m_a2 %>% 
  ...
  geom_line(aes(x = fraction_ocean_protected, y = value, col = m))+
  facet_wrap(vars(variable), scales = 'free', ncol = 1, strip.position = "left", 
              labeller = labeller(variable = c(avg_protection = "Average protection",
                                               delta_catch_a2 = "Delta catch (MMT)")))+
  scale_color_viridis_d()+
  labs(y = "", x = "Fraction of the ocean protected")+
  theme(strip.background = element_blank(),
        strip.placement = "outside",
        plot.title.position = "plot")+
  ggsave(filename = file.path(emLab_project_dir,"figures", "supplement","food_curves_by_m_a2.png"), 
         height = 6, width = 7)
```
...
### at 10% ocean protection

```{r}
delta_catch_per_stock_per_step_a1 <- read_rds(file.path(variant_dir, "delta_catch_per_stock_per_step_a1.rds"))

delta_catch_per_stock_at_10_percent_a1 <- delta_catch_per_stock_per_step_a1 %>% 
  group_by(stockid) %>% 
  slice(which.min(abs(fraction_protected - 0.1))) 
...
delta_catch_per_stock_at_10_percent_a1 <- delta_catch_per_stock_at_10_percent_a1 %>% 
...
delta_catch_per_stock_at_10_percent_a1 %>% 
  ...
  labs(title = "Delta catch per stock at 10% optimal ocean protection", 
       subtitle = "No displacement model")+
  scale_color_viridis_c()+
  ggsave(filename = file.path(variant_dir, "diagnostic_plots_at_10_percent_a1.png"), height = 10, width = 12)
```
...
delta_catch_per_stock_at_10_percent_a2 <- delta_catch_per_stock_per_step_a2 %>% 
  group_by(stockid) %>% 
  slice(which.min(abs(fraction_protected - .1))) 
...
delta_catch_per_stock_at_10_percent_a2 <- delta_catch_per_stock_at_10_percent_a2 %>% 
  left_join(stock_range_sizes) %>% 
  select(stockid, m, r, k, e , e_msy, e_over_e_msy, range_size,  msy,
         f_protected_a2, delta_catch_a2, delta_catch_a2_over_k, delta_catch_a2_over_msy)

delta_catch_per_stock_at_10_percent_a2 %>% 
  mutate(m = as.factor(m)) %>% 
  select(stockid, m, r, e, e_over_e_msy,  range_size) %>% 
  pivot_longer(-c(stockid, m, r), names_to = c("x_var"), values_to = "x_val") %>% 
  left_join(delta_catch_per_stock_at_10_percent_a2 %>% 
              mutate(delta_catch_a2 = delta_catch_a2/10^6) %>% 
  ...
  scale_color_viridis_c()+
  ggsave(filename = file.path(variant_dir, "diagnostic_plots_at_10_percent_a2.png"), height = 10, width = 12)
```

```{r}
delta_catch_per_stock_at_10_percent_a2 %>% 
  mutate(m = as.factor(m)) %>% 
  select(stockid, m, r, e_over_e_msy,  range_size) %>% 
  pivot_longer(-c(stockid, m, r), names_to = c("x_var"), values_to = "x_val") %>% 
  left_join(delta_catch_per_stock_at_10_percent_a2 %>% 
              mutate(delta_catch_a2 = delta_catch_a2/10^6) %>% 
              select(stockid, delta_catch_a2_over_k) %>% 
              pivot_longer(-c(stockid), names_to = c("y_var"), values_to = "y_val")) %>% 
  ggplot()+
  geom_point(aes(x = x_val, y = y_val, col = r, shape = m))+
  facet_grid(rows = vars(y_var), cols = vars(x_var), scales = 'free', switch ="both", 
             labeller = labeller(x_var = c(e = "Exploitation rate", e_over_e_msy = "E/E_msy", range_size = "Geographic range"),
                                 y_var = c(delta_catch_a2 = "Delta H", delta_catch_a2_over_k = "Delta catch / K", delta_catch_a2_over_msy = "Delta /MSY")))+
  labs(x = "", y = "")+
  theme(
    aspect.ratio = 1,
    strip.background = element_blank(),
  ...
  scale_color_viridis_c()+
    ggsave(filename = file.path(emLab_project_dir,"figures","supplement", "food_provision_diagnostic_plots_at_10_percent_a2.png"), 
           height = 4)
```
...
delta_catch_per_stock_at_5_percent_a1 <- delta_catch_per_stock_per_step_a1 %>% 
  group_by(stockid) %>% 
  slice(which.min(abs(fraction_protected - .05))) 
