...
$$\frac{dH}{dR} = \frac{e\times k \times m \times(m + e) \times ((e \times r + 2 \times e \times m) + m \times r - 2 \times e \times m)}{r \times(e \times R + m)^3} $$
```{r}
...
```{r}
estimate_slopes_a2 <- function(e = stocks_info$ex_rate, 
                               m = stocks_info$m,
                               r = stocks_info$r, 
                               k = stocks_info$k, 
                               R){
...
while(nrow(unprotected_matrix) > 0){
  ...
  slopes <- estimate_slopes_a2(R = current_state)
...
ggsave(delta_catch_curve_comparison_plot, file.path(variant_dir, "delta_catch_curve_comparison_a1_vs_a2.png"),
       width = 7, 
       height = 4)

ggsave(delta_catch_curve_comparison_plot, file.path(emLab_project_dir, "figures", "supplement", "delta_catch_curve_comparison_a1_vs_a2.png"), 
       width = 7, 
       height = 4)
```
...
ranking_diff %>% 
  ...
  ggsave(filename = file.path(emLab_project_dir, "figures", "supplement", "food_priorities_diff_a1_vs_a2.png"), dpi = 300,  height = 9, width = 8)
