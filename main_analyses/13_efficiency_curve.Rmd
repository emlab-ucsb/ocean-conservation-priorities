...
bio_equals_food <- efficiency_curve %>% 
  ...
  mutate(run_pattern = paste(effort_assumption, bio_x_factor, sep = "_"),
         caption =  paste0("biodiversity_equals_food_a", run_pattern, "x_bio_", round(fraction_protected, 2), "protected"),
         ranking_df = map(run_pattern, read_ranking_file),
         ranking_raster = map(ranking_df, create_ranking_raster,MPAs_df = MPAs_df, ocean_df = ocean_matrix, ocean_raster = ocean_low_res_moll),
         masked_raster = map2(ranking_raster, fraction_protected, .f = get_optimal_raster),
         optim_map = walk2(masked_raster, caption, .f = map_optimal_raster))

# bio_equals_food$masked_raster[bio_equals_food$effort_assumption == 2][[1]] %>%
#   writeRaster(file.path(emLab_project_dir, "other-project-materials", "natgeo-magazine", "equal_weights_bio_and_food.tif"))
```
