...
run_ranking <- function(effort_assumption){
  
  foreach(i = x_bios[1:6])%dopar%{
    ...
    multiobjective_feature_weigths <- calculate_multiobjective_weights(alpha_b = w_bio, alpha_c = 0, alpha_f = w_food,
...
system.time(test_run <- run_ranking(effort_assumption = 1))
```
...
foreach(i = x_bios[1:5])%dopar%{
  ...
  while(nrow(unprotected_matrix) > 0){
    ...
    best_cell_indeces <- doBy::which.maxn(delta, n = 10000)
