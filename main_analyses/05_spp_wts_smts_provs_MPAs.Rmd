...
map_priorities(ranking_raster_se, discrete = T, mpas_raster = MPAs, caption = "Biodiversity, scorched Earth", ocean_df = ocean_matrix) +
...
map_priorities(ranking_raster, discrete = T, mpas_raster = MPAs, caption = "Biodiversity, with impacts", ocean_df = ocean_matrix) +
...
ranking_diff %>% 
  ...
  colorspace::scale_fill_continuous_diverging(rev = T,
                                              guide = guide_colorbar(title = "ranking difference",
                                                                     ...
                                                                     keywidth = 0.3))+
  my_theme_map()+
  ggsave(filename = file.path(emLab_project_dir, "figures", "supplement", "bio_priorities_diff_scorched_earth.png"), 
...
HS_ranking <- ranking_raster %>% 
  mask(hs_mask, mask_value = 0, updatevalue = NA)

f_HS_in_mpa <- sum(!is.na(values(HS_MPAS_raster)))/sum(!is.na(values(HS_ranking)))

HS_bio_map <- map_priorities(HS_ranking, discrete = T, mpas_raster = HS_MPAS_raster, legend_title = "Top % of the High Seas", ocean_df = ocean_matrix,
                             legend_key_widths = c(f_HS_in_mpa, 0.1 - f_HS_in_mpa, rep(.1, 6), 0.3)/1.5)

EEZ_MPAs_raster<- MPAs %>% 
  mask(hs_mask, mask_value = 0, updatevalue = NA, inverse = T) 

EEZ_ranking <- ranking_raster %>% 
  mask(hs_mask, mask_value = 0, updatevalue = NA, inverse = T) 

f_EEZ_in_mpa <- sum(!is.na(values(EEZ_MPAs_raster)))/sum(!is.na(values(EEZ_ranking)))

EEZ_bio_map <- map_priorities(EEZ_ranking, discrete = T, mpas_raster = EEZ_MPAs_raster, legend_title = "Top % of Exclusive Economic Zones", ocean_df = ocean_matrix,
                              legend_key_widths = c(0.1*f_EEZ_in_mpa/0.05, 0.1*(0.05 - f_EEZ_in_mpa)/0.05, rep(.1, 6), 0.3)/1.5)
```
...
rep_today_vs_future <- avg_rep_today %>% 
  ...
  bind_rows(avg_rep_top_20 %>% 
              rename(rep = rep_top_20) %>% 
              mutate(scenario = "Protecting the most important 20% of the ocean")) 
...
rep_today_vs_future %>% 
  ggplot()+
  geom_histogram(aes(rep, fill = scenario),, show.legend = F)+
  geom_vline(data = rep_means, 
             aes(xintercept = mean_rep), col = "black", linetype ="dashed", size=1)+
  facet_wrap("scenario", nrow = 2)+
  scale_fill_brewer(palette="Dark2")+
  labs(x = "Fraction in MPA", y = "Number of provinces", col =  "")+
  ggsave(filename = file.path(emLab_project_dir, "figures","supplement","bio_provinces_representation_histogram.png"), 
         dpi = 300,  height = 5, width = 8)
