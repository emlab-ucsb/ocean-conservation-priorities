...
foreach(iso_code = unique(territories_df$iso_ter1)) %dopar% { 
  ...
  eez_cells <- unique(territories_df$cell_id[territories_df$iso_sov1 == iso_code])
...
a <- MPAs_df %>% 
  filter(!is.na(mpa)) %>% 
  select(cell_id) %>% 
  bind_rows(abnj %>% mutate(cell_id = as.character(cell_id))) %>% 
  ...
  replace(is.na(.), 0) %>% 
  select(-x, -y, -cell_id, -delta, -pick_order, -is_mpa, -eez_iso, -eez_id) %>% 
...
b %>% 
  ...
  geom_rect(aes(xmin = 0, 
                ...
                ymin = min(b$V)/sum(feature_wts), 
                ymax = max(b$V)/sum(feature_wts)),
