...
```{r}
territories_df <- raster(file.path(emLab_project_dir, "data", "02_processed", "masks", "territories.tif")) %>% 
...
territories_df %>% 
  group_by(mrgid_eez) %>% 
  summarize(n = n_distinct(iso_sov1)) %>% filter(n > 1)
```

```{r}
eez_ids <- unique(territories_df$mrgid_eez)

registerDoParallel(cores = 50) #register cores

foreach(eez_id = eez_ids) %dopar% { 
  
  # Normalize and trim 
  
  eez_iso <- unique(territories_df$iso_sov1[territories_df$mrgid_eez == eez_id])
  
  eez_cells <- territories_df$cell_id[territories_df$mrgid_eez == eez_id]
  ...
  ranking$eez_iso <- eez_iso
  
  ranking$eez_id <- eez_id
  
  data.table::fwrite(ranking, 
                     file.path(variant_dir, "national_results", paste0(eez_iso, "_results.csv")))
...
```{r}
list.files(file.path(variant_dir, "national_results"), pattern = "results", full.names = T)[61] %>% 
  read_csv() 
...
national_results <- list.files(file.path(variant_dir, "national_results"), pattern = "results", full.names = T) %>% 
  map_dfr(.f = ~.x %>% 
        ...
        arrange(desc(delta)) %>% 
        select(cell_id, eez_iso, delta) %>%
        tibble::rowid_to_column("pick_order")
  ) 

national_results %>% filter(eez_iso == "HRV")
```
...
representation_curves <- MPAs_df %>% 
  ...
  replace(is.na(.), 0) %>% 
  select(-x, -y, -cell_id, -delta, -pick_order, -is_mpa, -eez_iso) %>% 
...
benefit_curve %>% 
  ...
  ggsave(file.path(variant_dir, "benefit_curve_national_priorities.png"),
         width = 7, height = 4)
```
