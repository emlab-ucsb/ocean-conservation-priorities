...
territories_df <- territories_df %>% 
  replace_na(list(territory_id = 999,
                  iso_ter1 = "ABNJ",
                  name = "high seas",
                  mrgid_eez = 999,
                  iso_sov1 = "ABNJ")) 
...
```{r}
terri_ids <- unique(territories_df$territory_id)

registerDoParallel(cores = 10) #register cores

foreach(terri_id = terri_ids) %dopar% { 
  
  # Normalize and trim 
  
  terri_iso <- unique(territories_df$iso_ter1[territories_df$territory_id == terri_id])
  
  terri_cells <- territories_df$cell_id[territories_df$territory_id == terri_id]
  
  national_features_matrix <- features_matrix[terri_cells,  , drop = F]
  ...
  ranking$terri_iso3 <- terri_iso
  
  data.table::fwrite(ranking, 
                     file.path(variant_dir, paste0(terri_iso, "_results.csv")))
...
```{r}
national_results <- list.files(variant_dir, pattern = "results", full.names = T) %>% 
  map_dfr(.f = ~.x %>% 
        read_csv() %>% 
        arrange(desc(delta)) %>% 
        select(cell_id, terri_iso3, delta) %>%
...
representation_curves <- MPAs_df %>% 
  ...
  replace(is.na(.), 0) %>% 
  select(-x, -y, -cell_id, -delta, -pick_order, -is_mpa, -terri_iso3) %>% 
