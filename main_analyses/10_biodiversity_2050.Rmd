...
```{r, eval = F}
spp_files <- tibble(filepath = list.files(c("../Zonation/data/aquamaps_spp_distributions_2050/"), 
                                            full.names = T),
...
data.table::fwrite(bio_features_df, "data/spp_2050_df.csv")
```

```{r}
spp_2050_df <- data.table::fread("data/spp_2050_df.csv") %>% 
  as_tibble()

spp_2050_files <- tibble(filepath = list.files(c("../Zonation/data/aquamaps_spp_distributions_2050"), 
                                               full.names = T),
                         valid_sci_name = str_replace_all(str_remove(basename(filepath), ".tif"), "_", " "))
...
spp_info <- data.table::fread("data/spp_weights.csv") %>% 
  as_tibble() %>% 
  rename(w = spp_weight) %>% 
  mutate(new_file = str_replace(basename(filepath), "\\.tif", "_2050.tif")) %>% 
...
write_rds(ranking, 
          file.path(output_dir, "ranking_results_approach_1.rds"))
```
...
```{r}
ranking <- read_rds(file.path(output_dir, "ranking_results_approach_1.rds"))
...
benefit_curve %>% 
  ...
  labs(x = "Fraction Ocean Protected",
       ...
       caption = "Biodiversity + Climate Change, approach 1, Juanation")+
  theme_classic()+
  ...
  ggsave(file.path(figs_output_dir, "benefit_curve_approach_1.png"),
         width = 7, height = 4)
```
...
writeRaster(ranking_raster, 
            file.path(output_dir, "ranking_raster.tif"), 
            overwrite = T)
```

```{r}
ranking_raster <- raster( file.path(output_dir, "ranking_raster.tif"))
...
tmap::tmap_save(ranking_map, 
                filename = file.path(figs_output_dir, "priority_map_approach_1.png"),
...
```{r}
raster_today <- raster("05_spp_wts_smts_provs_MPAs/results/ranking_raster_with_impacts.tif")
...
diff_map <- comparison_raster %>%
  ...
  tmap::tm_raster(title = "",
                  palette = "seq",
                  style = "cat",
                  labels = rev(c("irreplaceable (top 10%)", "increasing", "comparable", "decreasing", "least (bottom 50%)")),
                  legend.is.portrait = T, 
                  legend.reverse = T)+
  tmap::tm_shape(land_shp_moll)+
...
tmap::tmap_save(diff_map, 
               filename = file.path(figs_output_dir, "map_2050_vs_today_approach_1.png"),
                dpi = 300,  height = 4, width = 8 )
...
write_rds(ranking, 
          file.path(output_dir, "ranking_results_approach_2.rds"))
```
...
```{r}
ranking_approach_2 <- read_rds(file.path(output_dir, "ranking_results_approach_2.rds"))
...
benefit_curve_approach_2 %>% 
  ...
  geom_rect(aes(xmin = 0, 
                ...
                fill = "#69B578", alpha = 0.01)+
  ggsave(file.path(figs_output_dir, "benefit_curve_approach_2.png"),
         width = 7, height = 4)
```
...
writeRaster(ranking_raster_approach_2, 
            file.path(output_dir, "ranking_raster_approach_2.tif"), 
            overwrite = T)
```
...
tmap::tmap_save(ranking_map_approach_2, 
                filename = file.path(figs_output_dir, "priority_map_approach_2.png"),
                dpi = 300,  height = 4, width = 8 )
```
...
comparison_raster_approach_2 <- overlay(ranking_raster_approach_2, raster_today, fun = function(x, y){if_else(x >= 0.95 & y >= 0.95, 1, 
                                                                            if_else(x <= 0.5 & y <= 0.5, -1, 
                                                                                    if_else(abs(x-y) <= 0.01, 0, 
                                                                                            if_else(x > y, 0.5, -0.5))))})
...
tmap::tmap_save(diff_map_approach_2, 
               filename = file.path(figs_output_dir, "map_2050_vs_today_approach_2.png"),
                dpi = 300,  height = 4, width = 8 )
