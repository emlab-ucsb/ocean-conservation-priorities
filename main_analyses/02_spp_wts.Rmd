...
features_df <- data.table::fread(file.path(emLab_project_dir, 
                                           "data", "02_processed", "species_distributions", "all_spp_distributions_df.csv")) %>% 
  as_tibble()
...
aquamaps_weights <- aquamaps_spp_info %>% 
  ...
  arrange(valid_sci_name) %>% 
  select(valid_sci_name, spp_weight)
...
spp_weights <- bind_rows(aquamaps_weights, 
                         seabirds_weights)
...
ranking_map_se <- tmap::tm_shape(ranking_raster_se)+
  ...
  tmap::tm_credits("spp + wts, scorched Earth") +
...
ranking_map <- tmap::tm_shape(ranking_raster)+
  ...
  tmap::tm_shape(land_50_moll)+
  tmap::tm_fill(col = "black", border.col = "transparent")+
  tmap::tm_credits("spp + wts, with impacts") +
  tmap::tm_layout(title = "Biodiversity Conservation Priorities",
                  title.position = c("center", .95),
                  inner.margins = c(0.12, 0, 0.08, 0.04), 
                  frame = F,
