...
```{r}
co_benefits_biodiversity <- read_csv("../Juanation/11_multiple_objectives/results/co_benefits_biodiversity.csv")
co_benefits_food <- read_csv("../Juanation/11_multiple_objectives/results/co_benefits_food.csv")
co_benefits_carbon <- read_csv("../Juanation/11_multiple_objectives/results/co_benefits_carbon.csv")
```


```{r}
co_benefits_matrix <- co_benefits_biodiversity %>% 
  slice(which.min(abs(fraction_protected - 0.1))) %>% 
  mutate(priority = "biodiversity") %>% 
  bind_rows(co_benefits_carbon %>% 
              slice(which.min(abs(fraction_protected - 0.1))) %>% 
              mutate(priority = "carbon")) %>% 
  bind_rows(co_benefits_food %>% 
              slice(which.min(abs(fraction_protected - 0.1))) %>% 
              mutate(priority = "food")) %>% 
  select(priority, biodiversity, carbon, food) %>% 
  mutate_if(is.numeric, ~./(max(.)))
```

```{r}
radar_top10 <- ggradar::ggradar(co_benefits_matrix, 
                                base.size = 3,
                                font.radar = "sans",
                                axis.label.size = 3,
                                grid.label.size	= 3,
...
```{r}
# multi_objective_matrix <- read_rds("../Juanation/11_multiple_objectives/results/multi_objective_norm_features_diff_matrix.rds")
# 
# bio_ranking <- read_rds("../Juanation/05_spp_wts_smts_provs_MPAs/results/ranking_results_with_impacts.rds")
# carbon_ranking <- read_rds("../Juanation/06_carbon/results/ranking_results_with_impacts.rds")
# food_ranking <- read_rds("../Juanation/07_food/results/ranking_results.rds")
```
...
multi_objective_curves_biodiversity_priority <- co_benefits_biodiversity %>% 
  ...
  scale_color_manual(values = c("#55ae95", "#3f4d71", "#fd7792")
                       #wesanderson::wes_palette("Darjeeling1", n = 3)
                     )
  wesanderson::wes_palette("Darjeeling1", n = 3)
  paletteer::scale_color_paletteer_d(package = "ggsci", palette = "default_locuszoom", direction = 1)
...
fig2 <- overlap_map_90_percent / (multi_objective_curves_biodiversity_priority | multi_objective_curves_carbon_priority | multi_objective_curves_food_priority) + plot_layout(ncol = 1, heights = c(2,1)) + plot_annotation(tag_levels = "a")

ggsave(fig2, filename = "figure_2.pdf", dpi = 300,  height = 6, width = 8)
ggsave(fig2, filename = "figure_2.png", dpi = 300,  height = 6, width = 8)
```
