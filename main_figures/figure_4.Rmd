...
```{r}
food_priorities_raster <- raster(file.path(emLab_project_dir, "data", "03_output", "07_food", "ranking_raster_a2.tif"))
delta_food_V_raster <- raster(file.path(emLab_project_dir, "data", "03_output", "07_food", "delta_v_raster_a2.tif"))
...
stats_by_country$country[stats_by_country$iso3 == "FSM"] <- "FSM"

stats_by_country$country[stats_by_country$iso3 == "SJM"] <- "SJM"

stats_by_country$country[stats_by_country$iso3 == "MMR"] <- "Myanmar"

stats_by_country$country[stats_by_country$iso3 == "GNQ"] <- "Eq. Guinea"

stats_by_country$country[stats_by_country$iso3 == "ARE"] <- "UAE"

stats_by_country$country[stats_by_country$iso3 == "CCK"] <- "Keeling Isl"

stats_by_country$country[stats_by_country$iso3 == "PYF"] <- "PYF"

stats_by_country$country[stats_by_country$iso3 == "CPV"] <- "CPV"

stats_by_country$country[stats_by_country$iso3 == "GNB"] <- "GNB"

stats_by_country$country[stats_by_country$iso3 == "GLP"] <- "GLP"
```
...
make_propeller_plot <- function(stats_by_country, objective, top_n_countries){
  ...
  top_countries <- stats_by_country %>% 
    ...
    ungroup() %>% 
    top_n(top_n_countries, sum_delta_v_10_percent) %>% 
    pull(country)
...
propeller_bio_plot <- make_propeller_plot(stats_by_country = stats_by_country, objective = "biodiversity", top_n_countries = 100)
...
make_propeller_plot(stats_by_country = stats_by_country, objective = "carbon", top_n_countries = 100) %>% 
  ggsave(filename = file.path(emLab_project_dir, "figures", "supplement" ,"propeller_carbon.png"), 
         dpi = 300,  height = 13, width = 13)

make_propeller_plot(stats_by_country = stats_by_country, objective = "food", top_n_countries = 100) %>% 
